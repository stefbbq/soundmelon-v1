class ArtistInvitation < ActiveRecord::Base
  belongs_to :user
  belongs_to :artist

  validates :email, :presence => true # :uniqueness => { :scope => :artist_id, :case_sensitive => false, :message => 'has already been invited' }
  ...
  def send_invitation
    ...
    if receiver
      ...
      msg       = "Your artistmate #{sender.get_full_name} has invited you to join #{artist.name} at soundmelon."
  ...
  def self.create_invitation_for_artist invitee_user, artist, email_user_name_hash_arr
    ...
    email_user_name_hash_arr.each{|item|
      ...
      if user_email
        invitation = self.find_by_artist_id_and_email artist.id, user_email
        unless invitation
          self.create(
  ...
  def self.create_invitation_for_artist_and_fan invitee_user, artist, invited_user, access_level = 0
    invitation = self.find_by_artist_id_and_email artist.id, invited_user.email
    unless invitation
      self.create(
        :user_id      => invitee_user.id,
        :access_level => access_level,
