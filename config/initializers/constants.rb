NO_OF_FOLLOWER_TO_DISPLAY         = 5
NO_OF_FOLLOWING_TO_DISPLAY        = 5
DEFAULT_NO_OF_MESSAGE_DISPLAY     = 6
FOLLOWING_FOLLOWER_PER_PAGE       = 10
SEARCH_ARTIST_RESULT_PER_PAGE     = 10
SEARCH_FAN_RESULT_PER_PAGE        = 10
READ                              = 1
UNREAD                            = 0
MENTION_COUNT_FOR_LAST_N_HOURS    = 48
POST_PER_PAGE                     = 15
MESSAGES_PER_PAGE                 = 6
WELCOME_MSG                       = 'welcome to the beta'

USER_TYPE_ADMIN                   = 'admin'

FEEDBACK_SUBJECTS                 = [
  "Music Player", "Search", "Fan Social", "Artist Social", "Music Publication", "Photo Publication",
  "Shows Publication", "Buzz", "Profile management", "Notifications", "Invitation System", "Artistmates Management"
]

ARTIST_MUSIC_SHOW_LIMIT   = 4
ARTIST_PHOTO_ALBUM_SHOW_LIMIT  = 4
ARTIST_FAN_SHOW_LIMIT          = 5
ARTIST_MEMBER_SHOW_LIMIT       = 5
ARTIST_FOLLOWER_SHOW_LIMIT     = 5
FOLLOWING_ARTIST_SHOW_LIMIT  = 10
FOLLOWING_FAN_SHOW_LIMIT     = 5
FOLLOWER_SHOW_LIMIT          = 5
SHOW_DATE_SHOW_LIMIT         = 4
ARTIST_FEATURED_SONG_LIMIT   = 6
ARTIST_CONNECTION_SHOW_LIMIT = 5
ARTIST_CONNECTION_PER_PAGE   = 10
NO_FAN_MESSAGE               = "no fans yet"
NO_FOLLOWING_MESSAGE         = "not following anyone"
NO_FOLLOWING_ARTIST_MESSAGE  = "not following any artist"
NO_FOLLOWER_MESSAGE          = "no followers yet"
NO_SONG_ALBUM_MESSAGE        = "no music yet"
MO_FEATURED_SONGS_MESSAGE    = "no featured songs"
NO_PHOTO_ALBUM_MESSAGE       = "no photos yet"
NO_ARTIST_MEMBER_MESSAGE     = "no artist members"
NO_SHOW_DATE_MESSAGE         = "no upcoming shows"
NO_CONNECTION_MESSAGE        = "no connections yet"


# application code specific
CURRENT_TRACK_VAR_NAME       = 'curItNum'